...
```{r}
duniform_slow <- function(x_vals, a, b){
  result <- NULL
  len <- length(x_vals)
  for(i in 1:len){
    x = x_vals[i]
    result[i] <- ifelse(a <= x & b >= x, 1/(b-a), 0)
  }
  return(result)
}
```
...
```{r}
duniform(c(1,2,3,4,5,6,7,8), 3, 7)
```
...
```{r}
microbenchmark::microbenchmark(duniform_slow( seq(-4,12,by=.0001), 4, 8), times=100)
```
...
```{r}
microbenchmark::microbenchmark(duniform_fast( seq(-4,12,by=.0001), 4, 8), times=100)
```
the fast function is significantly faster... who would have guessed!
...
```{r}
duniform_defaults <- function(x, a = 0, b = 1){
  result <- ifelse(a <= x & b >= x, 1/(b-a), 0)
  return(result)
}
```
